...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \begin{abstract}
    Systems programming involves dynamically requesting memory that in the C++ language has to be manually managed by the programmer. 
    However, this leaves the opportunity for exploitable memory safety errors.
    Modern C++ recommends that smart pointer are used to automatically manage resources such as dynamic memory.
    However, many legacy programs may have been written before these features were introduces to the language.
    We developed the ptr-tidy tool in order to automatically rejuvenate such programs to use smart pointers.
    The tool uses the Clang and LLVM libraries in order to create a componentised and powerful design which could be reused by other refactoring or rejuvenation tools. 
    We present a case study and and apply the tool on a number of open-source C++ libraries to show that the tool can successfully rejuvenate non-trivial libraries and that the tool can be used on libraries from many areas of computer science. 
    We discuss in which circumstances the tool performs well and the current limitations.
    \end{abstract}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    This is done when the amount of memory that we need at runtime cannot be know at the time of compilation of the program.
    ...
    \begin{itemize}
        \item \textbf{Dangling pointers}, prematurely freeing an object that is still in use.
        \item \textbf{Double free}, deleting an object multiple times.
        \item \textbf{Never free}, never deleting an object.
    \end{itemize}
    ...
    Unfortunately, simply not using these languages is not an option since their unmanaged nature makes them highly performant, and therefore the best option for systems such as a web browser.
    ...
    We therefore propose a solution in the form of a tool, \emph{ptr-tidy}~\cite{Usov2021}, to analyse and automatically refactor programs to follow the recommended use of smart pointers where the analysis can determine that such a change is syntactically correct and does not change the runtime behaviour of the program. 
    
    \begin{listing}
        \inputminted{c++}{code/leak.cpp}
        \inputminted{c++}{code/fix-leak.cpp}
        \caption{Example of a memory leak using manual management and how smart pointers correct this. Lines 4 and 5 show how interruptions in the program control flow can mistakenly cause memory leaks and how smart pointer correct this by automatically inserting all needed delete statements at compile time. All necessary headers are omitted for brevity}
        \label{lst:motivating-example}
    \end{listing}
    
    We provide a motivating example in \autoref{lst:motivating-example} where the first version of the function could have several control flow executions where the \texttt{Circle} object will leak.
    The example seems quite trivial, but can suddenly be quite non-trivial if the conditions in the \texttt{if} statement become calls into external libraries, which may or may not throw an exception.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    We exactly aim replace outdated coding patterns with newer, higher-level abstractions, as well as fitting the notion that this tool would be applied once to a codebase, rather than be a reoccurring task.
    We will however use the terms rejuvenation and refactoring interchangeably in this paper as it is a term that most developers are familiar with.
    
    We believe research in this area is going to increase, as languages move and evolve much quicker than can be supported in enterprise environments, resulting in accumulating technical debt. 
    This is supported by the fact that other relevant work has been completed on this subject.
    
    First and foremost, we have evidence that this is a problem that exists in industry through a paper by \citet{Wright2013} which describes a tool used at Google for running such rejuvenations across a large codebase. 
    \citet{Wright2013} show an example of running a simple rejuvenation of upgrading an API call.
    They use Clang for the parsing of code into an AST and traverse the tree to identify and perform simple refactorings.
    ...
    The former also uses the Clang and AST driven approach like \citet{Wright2013}, while the latter uses a representation called IPR~\cite{Dos2011}, which is a general and efficient data structure for representing C++ programs.
    We have not evaluated IPR as a representation compared to the Clang AST, but we can however comment that whilst we appreciate the aim of the project to create a efficient and compiler-independent representation, the greater amount of documentation that Clang has, as well as it being the choice of several other successful projects leads us to believe that it may be the more suitable and mature option for any such research.
    
    There has also been industry interest in automatic rejuvenation tools between languages rather than within the same language. 
    This is done either to increase the use of a new, improved language by creating creating tools to aid transition, or in order to reuse existing compilers.
    ...
    Similarly, several new languages such as Typescript and Dart have been recently developed which were designed not to be used by themselves, but to be transpiled to the popular Javascript language.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    The stack is an small memory store automatically managed for us by the compiler and is suitable for placing small, fixed sized parameters and arguments, such as perhaps loop counter variables.
    However, when objects must live beyond the lifetime of the current function call or if we do not know the size of an object until runtime, then we must storage these objects on the heap, occasionally called the \emph{free store}. 
    ...
    In the simplest case, this means that when a resource exists the scope of a function or method, it is released automatically.
    
    The first type of smart pointer are \emph{unique pointers} which have only a single unique owner at any time and therefore are ideal for resources which may not be copied, such as perhaps a global session dictionary.
    We can see how unique pointers and RAII work from out motivating example in  \autoref{lst:motivating-example}.
    Even though the \texttt{Circle} object is placed on the heap, the compiler can manage the smart pointer for us so that in all cases where the lifetime of the object ends, the object will be freed and ensures the resource will not leak. 
    ...
    As seen in \autoref{sec:background}, much similar research uses Clang and LLVM.
    LLVM~\cite{Lattner2004} is a compiler infrastructure project consisting of several compiler and toolchain technologies. 
    LLVM is centrally designed around a language independent intermediate representation (IR), which can be created by a number of language front-ends and is used by a number of architecture back-ends to generate machine code. 
    Program optimisations can then be applied by portable and reusable transformations on the IR.
    Any improvement in back-end code generation, as well as any new or improved IR optimisers can then benefit any of the supported language front-ends.
    It is therefore visible that this allows for a very modular and extensible design which can be seen in \autoref{fig:llvm-design}.
    
    Clang~\cite{Lattner2008} is such an LLVM front-end for C, C++ and a variety of other similar languages such as OpenGL.
    ...
    Further more, Clang also explicitly provides the \emph{LibTooling} C++ library for the creation of tools that leverage the Clang parsing front-end and AST.
    The Clang AST is different from ASTs produced by some other compilers in that it closely resembles both the written C++ code and the C++ standard~\footnote{\url{https://clang.llvm.org/docs/IntroductionToTheClangAST.html}}.
    ...
    The LLVM IR represents a language-independent, static single assignment (SSA)~\cite{Rosen1988} representation of the program.
    Variables in in SSA form can only be assigned once, which allows us to efficiently generate compact definition-usage graphs, which enable developers to simplify many dataflow optimizations and enables fast, flow-insensitive algorithms~\cite{Lattner2004}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    The first capability our tool would need is to be able to parse C++ code.
    ...
    The information in \autoref{subsec:clang-and-llvm} outlines why Clang is the most suitable parser to use, with the other benefit that it brings such as \emph{LibTooling}. 
    
    \subsection{Analysis}\label{subsec:methodology-analysis}
    
    Next, our tool would need to be able to identify specifically which pointers are suitable for refactoring.
    An \emph{unique pointer} is a smart pointer object that represent a single unique owner of a region of memory at any time. 
    We will therefore need to develop a static analysis algorithm to identify the maximum number of owners a region of memory has throughout all points of a program. 
    If a memory region has at most a single unique owner at a time, we can refactor the initialising raw pointer to a unique pointer, otherwise the pointer can be refactored to a shared pointer.    
    ...
    A key intuition is that a region of memory, subsequent to its initialisation, is only able to gain additional owners if access to the region of memory is \emph{shared} to another variable, that is if a pointer to this memory is copied to a another pointer, at which point the original owner is longer the unique owner.
    ...
    If it does not escape, meaning it is only used locally in the method, then it can be allocated on the stack rather than the heap as a performance optimisation.
    ...
    \begin{itemize}
        ...
        \item a pointer is does not escape but access to the memory is shared rather than transferred to a local variable. 
    \end{itemize}
    ...
    However, much of the existing work from \autoref{sec:related-work} which use Clang perform both their analyses and their source code refactoring on the Clang AST.
    We however propose an improved novel design which uses the Clang AST for parsing and refactoring and the LLVM IR for simple and efficient analysis.
    
    For the same input file, we can use the \emph{LibTooling} library to generate an AST and IR in-memory representation.
    Then, for any variable declaration (\emph{VarDecl}) in the Clang AST which allocates new memory on the heap, we locate this declaration in the IR.
    This is possible as we can instruct Clang to not discard variable names using a \texttt{fno-discard-value-names} flags, and as the combination of function signature and variable name in the translation unit (\emph{cpp file}) must be unique according to the One-definition rule~\cite{C++17}. 
    ...
    In \autoref{fig:code-to-ast-example}, given the variable declaration for the variable \texttt{a}, then as it is not a global variable, we can walk its parent nodes until we reach a \emph{FunctionDecl}.
    We are now able to use the function name to find the corresponding function declaration in the IR module, which can be seen in  \autoref{lst:code-to-ir}.
    ...
    Given a function in the IR, we can use its symbol table to use the declaration name to find the IR value on which we will perform our analysis.
    ...
    For a \texttt{false} result, the value declaration is refactored into a unique pointer, otherwise a shared pointer.
    However, the rewriting is identical for both cases with just the string tokens that we replace for the original type differing.
    ...
    The five cases of code (\emph{or rewriting rules}) that needs to be rewritten are:
    \begin{itemize}
        \item Declaration types.
        \item Variable initialisation and assignment expressions.
        \item Pointer delete statements.
        \item Function return types.
        \item Pointer Usages.
    \end{itemize}
    
    Rewriting declaration types are the easiest of all. We can directly replace the type of the variable declaration node that we already have access to. 
    For example for \mintinline{c++}{int *ptr}, we can extract the type of the variable, and format it into the string \mintinline{c++}{"std::shared_ptr<{}>"} or \mintinline{c++}{"std::unique_ptr<{}>"}, where the \texttt{\{\}} represent the replacement field.
    
    Initialisation and assignment expressions are less simple due to two reason.
    ...
    \end{itemize}
    For pointer variables, this is convoluted even more depending on if the pointer gets initialised using an existing pointer \mintinline{c++}{int* a = nullptr; int *b = a;}, or if it gets initialised using a new statement \mintinline{c++}{auto p = new Circle{Point{0,0},10}}.
    ...
    Otherwise, we can create a smart pointer from a pointer value using by the default equal sign copy initialiser or parentheses expression list. 
    
    Rewriting pointer deletion statements also require us to traverse the AST to find \mintinline{c++}{delete} statements for the variable declaration node that we are rewriting.
    However, they do not require any complicated logic, and we can upon finding relevant statements remove them from the source code.
    ...
    We explicitly check for each variable declaration that we handle if it gets returned, and if so, we can rewrite the function return type the same way that we handle variable declaration types. 
    
    Lastly, smart pointer have overloaded operators for the arrow \texttt{->} and dereference \texttt{*} operators so that they behave identically as if the value was a raw pointer.
    This simplifies our task as it reduces the amount of rewriting we have to do for each usage of a specific pointer value.
    However, there is no such overloaded operator for passing smart pointer as function arguments. 
    In this case, the compiler will find a type error, when it might expect a type such as \mintinline{c++}{int *}, but actually receives \mintinline{c++}{std::unique_ptr<int>}. 
    Therefore, we again have to traverse the AST to find all function call expressions which involve the variable declaration that we are currently handling, and we need to add \mintinline{c++}{get()} calls to the smart pointer values to get a raw pointer to the managed objects.
    
    We use the Clang \emph{Rewriter}\footnote{\url{https://clang.llvm.org/doxygen/classclang_1_1Rewriter.html}} class as a high level abstraction compared to handling files and text buffers ourselves, which would likely be error-prone for buffer overflows.
    ...
    \begin{figure}
        ...
        \caption{An example of all the rewriting rules except pointer delete statements. This example solely shows rewriting and does not necessarily follow respect any smart pointer viability analyses. We see how both the declaration and equal sign copy initialisation of \texttt{a} have been rewritten to into a \texttt{shared\_ptr}. All the uses of \texttt{a}  that require a raw pointer have a \texttt{get()} call added to obtain the managed memory.}
        \label{fig:rewriting}
    \end{figure}
    ...
    It is visible how in the majority of cases, the first two rules will be applicable as every new smart pointer need to have the correct type and be initialised to the right type of object. 
    The fifth rule will also likely be applicable in most cases as variables tend to not be unused and variables which were pointers will be used in functions expecting pointers, as they otherwise could be stack variables.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Through our research, we would like to answer the question of whether our implemented tool can truly improve C++ code by seeing if the tool can be used outside small examples on larger existing programs, as well as if the refactored results produce code which can be compiled and can be judged to be better.
    ...
    \begin{itemize}
        \item \textbf{Hypothesis 1} The automatic translation of raw pointers into smart pointers can be used to make existing C++ code more modern, safe and extendable.
    ...
    We run our experiments on a Windows 10 Pro x64 machine using the Windows Subsystem for Linux, with an AMD Ryzen 5 PRO 4650U and 16 GiB RAM, using TinyXML-2 v8.0.0.
    ...
    In order to answer hypothesis 1, it was decided to run a case study instead of applying the tool in large on a range of open source projects. 
    This was decided as an analysis into a well chosen target project will be able to show relevant success and failure cases that we can extrapolate onto more general C++ projects of the same kind. 
    If the impact of the success cases outweighs the the faults of the failure cases, then we will be able to judge the our hypothesis as being true.
    
    We decided on using the \emph{TinyXML-2} library as our chosen case study~\footnote{\url{https://github.com/leethomason/tinyxml2}}.
    ...
    It is also for example used in the \emph{libigl}~\cite{Libigl} geometry processing library,  which itself is used by a variety of large industry companies.
    ...
    What makes library a particularly attractive choice is the simplicity of the library.
    ...
    The library is also around three thousand lines of code in length, making it large enough to likely have a variety of edge cases, whilst remaining small enough for manual inspection.
    ...
    We can in \autoref{lst:tinyxml-refactor-example} an example of a method from the library which would be well suited for refactoring to use smart pointers.
    The heavy use of pointers and memory allocation is what leads us to believe that the case study into this library will generalise well onto all other C++ projects.
    ...
    The user of our tool can easily generate a compilation database for the project that they wish to rejuvenate using their build tool, such as \emph{CMake}.
    ...
    We can first and foremost see in \autoref{lst:tinyxml-refactor-example-refactored} the result of the rejuvenation our candidate method in \autoref{lst:tinyxml-refactor-example}.
    
    We can see that we achieve a similar refactoring as in \autoref{fig:rewriting}. 
    The smart pointer is initialised using using the pointer copy initialiser.
    ...
    If we enable Clang compiler optimisations when generating the IR, the compiler correctly identify that \texttt{p} cannot escape within the function, giving it a signature in the IR of 
    \mintinline{llvm}{
    (i32* nocapture %p)
    }.
    ...
    What is possible is that we can manually mark a variable in the code as being non-capture and non-alias, such as:
    \begin{minted}{c++}
    ...
    Such code therefore is not portable and not standard conforming, making it very unlikely unacceptable for most projects.
    ...
    The impact this on our tool is that the extent of our analysis is not as complete as we would like it to be, however \autoref{lst:tinyxml-refactor-example-refactored} and \autoref{fig:rewriting-example2} show that we can still achieve desirable results through the analysis of local variables. 
    ...
    The rejuvated version of the TinyXML-2 library that the tool produces is not directly compilable into an object file, mainly due to edge cases in the rewriting component. 
    These mainly stem from outdated C++ practises that must be explicitly handled in modern code that has more strict type checking. 
    ...
    therefore causes a type error during compilation.
    ...
    The fixes that need to be applied can be done by the programmer, with all of the identified fixes in the TinyXML-2 library being a single line in length. 
    
    We do not think this however detracts from the value or usefulness of our tool.
    Our tool provides fully rejuvenated and compilable programs without any human input needed for simpler inputs, using modern styles of C++.
    For larger, more complex, older input programs some human input is currently still needed, however the work of our tool still allows the programmer to make their programs more modern and safer in by performing the complex analysis and guiding the programmer towards the needed fixes through compiler errors.
    This results in the rejuvenated output being obtained easier by removing the need for the programmer to manually understand the structure and style of the programmer and the often complex ownership relations between the various variables in the program. 
    
    We therefore believe that the analysis of our tool on the case study of the TinyXML-2 library, even with the identified failure cases, the success of the automated translation and identified success cases allow us to confirm our first hypothesis.
    
    \subsection{Analysis of performance on a corpora of projects}
    
    In order to answer hypothesis 2, it was decided to run analysis of the tool on more various open source projects and gather metrics on the performance of the tool in terms of its performance in successfully identifying pointers, how extensive the rejuvenation in context of the whole program, and some more general runtime characteristics. 
    
    It was decided that whilst gathering more projects to test, we would continue to focus on simple, single file libraries such as TinyXML-2 in order to ease the cost of setting up each project to be used by the tool. 
    ...
    This list also ensures that we only select projects which we are able to use and modify as well as including project from many areas of software engineering such as graphics and geometry to file parsers.
    ...
    \end{tabular}    
    \caption{Data gathered from the rejuvenation of each of these projects. Total lines of code include both the implementation and header file of each project. Runtime measured in milliseconds as the median of five runs.}
    \label{table:data}
    \end{table*}
    ...
    \begin{figure*}
        ...
        \caption{First graph shows the fraction of non-escaped, and therefore refactored, pointers for various projects. This varies from a minimum of 30\% up to a maximum of 60\%, with a mean of 42\%. The second graph shows the number of total pointers compared to the number of total lines in each project.}
    ...
    \begin{figure*}
        ...
        \caption{Graphs of the tool runtime versus the total lines of code in each project and the total number of pointers that was analysed. The x axis for both graphs are plotted on a log scale. This is for clarity purposes in order to emphasise the two outliers consisting of the Clipper and Xatlas libraries. Time was measured in microsecond using the Bash \texttt{time} command and measuring the median real execution time of 5 runs.}
        \label{fig:time}
    \end{figure*}
    ...
    The mean of 42\% is shows that we are able to identify a non-trivial amount of pointers, however it is unclear this is not higher because of a fault in the analysis or whether this is the typical performance for a conservative analysis.
    ...
    We therefore conclude that the length of a project that the tool has to rejuvenate has no impact on its performance. 
    
    \autoref{fig:lines} the number of lines that were modified in the rejuvenated version of the project compared to the original. 
    We can much more clearly see our previous expectation that some projects utilise pointer more and other less. 
    ...
    We were unable to establish why these libraries take an exceptionally longer time to process than the TinyXML-2 library which has the same amount of lines of code and analysed pointers as the Clipper library. 
    
    To conclude, we believe we can partially confirm our second hypothesis.
    We show that the tool generalises to a range of other libraries apart from the TinyXML-2 library selected in our case study and show that our analysis achieves similar results across these libraries. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Smart pointer create memory or resource handles which automatically get released when the handle exists the scope of a function or method. 
    ...
    The learning and application of new language requires a significant time investment that may not be able to be allocated. 
    
    We then introduce our \emph{ptr-tidy} tool which was designed automatically rejuvenate code that does not use smart pointers, thereby solving our first problem.
    We show the design and implementation of the tool, which uses the Clang and LLVM libraries. 
    The tool uses Clang and the Clang abstract syntax tree (AST) in order to obtain a rich and closely resembling representation of an input program.
    ...
    This design is the solution to our second problem, as we believe this design is both generic, reusable and powerful enough to be used for a variety of same language refactoring or rejuvenation or different language transpiling.
    ...
    We also discovered that due to limitations in the specification of the C++ language, we are unable to rejuvenate function parameters statically, which meant the extend of our rejuvenation had limits.
    ...
    Most importantly, the authors present an interprocedural analysis which we hypothesise could bring greater effectiveness to our tool and therefore we believe that it could lead to improved analysis performance compared to our implemented intraprocedural analysis. 
    The similarity of Java and C++ as object oriented languages allowed for a direct application of the algorithm to C++.
    ...
    However, it remains to investigate whether these optimisations can be called dynamically during the running of our tool in order to gain information about conservative assumptions that we currently make. 
